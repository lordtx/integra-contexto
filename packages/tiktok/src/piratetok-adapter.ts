  ...
  return {
    ...
    async connect(config: TikTokAdapterConfig, handlers: TikTokAdapterEvents): Promise<void> {
      ...
      client.on(EventType.chat, (data) => {
        handlers.onChat({
          event: 'chat.message', platform: PLATFORM,
          streamId: lastStreamId || streamerUsername,
          user: { id: data.user?.uniqueId || 'unknown', username: data.user?.uniqueId || 'unknown', displayName: data.user?.nickname },
          message: data.content || '', timestamp: Date.now(),
        });
      });
      client.on(EventType.follow, (data) => {
        handlers.onFollow({
          event: 'follow', platform: PLATFORM, streamId: lastStreamId || streamerUsername,
          user: { id: data.user?.uniqueId || 'unknown', username: data.user?.uniqueId || 'unknown', displayName: data.user?.nickname },
          timestamp: Date.now(),
        });
      });
      client.on(EventType.gift, (data) => {
        handlers.onGift({
          event: 'gift', platform: PLATFORM, streamId: lastStreamId || streamerUsername,
          user: { id: data.user?.uniqueId || 'unknown', username: data.user?.uniqueId || 'unknown', displayName: data.user?.nickname },
          giftName: data.gift?.name || 'unknown', diamondCount: data.gift?.diamondCount ?? 0,
          repeatCount: data.repeatCount ?? 1, timestamp: Date.now(),
        });
      });
